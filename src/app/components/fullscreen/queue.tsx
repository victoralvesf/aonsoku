...
export function FullscreenSongQueue() {
  ...
  const queue = [...currentSongList].slice(currentSongIndex, currentSongList.length)
    ...
    <Table className="h-full mb-1">
      <TableBody className="rounded-md">
        {queue.map((entry, index) => (
          <TableRow
            key={entry.id}
            className={cn("hover:bg-muted-foreground/15 border-0 cursor-pointer", currentSongId === entry.id && "bg-primary/25")}
            onClick={() => {
          ...
          >
            <TableCell className="w-[30px] text-center font-medium">{currentSongIndex + (index + 1)}</TableCell>
            <TableCell>
